...
public class SendSmsController {
	...
	public SendSmsController(Activity context, final Dialog dialog, View view, Pay payBean,
		...
		bt_submit = (Button) this.view.findViewById(CommonUtil.getResourcesId(context, "bt_submit", "id"));
		bt_cancel = (Button) this.view.findViewById(CommonUtil.getResourcesId(context, "bt_cancel", "id"));
		TextView tv_desc = (TextView) this.view.findViewById(CommonUtil.getResourcesId(context, "tv_desc", "id"));
		...
		context.registerReceiver(new BroadcastReceiver() {
			...
			public void onReceive(Context _context, Intent _intent) {
				if (is_send_sms) {
					...
					// Toast.LENGTH_SHORT).show();
					// 弹出支付成功对话框
					int layout_id = CommonUtil.getResourcesId(_context, "dialog_pay_success", "layout");
					int style_id = CommonUtil.getResourcesId(_context, "QidaDialog", "style");
					QidaDialog dialog = new QidaDialog(context, layout_id, style_id,
							responseCallback);
