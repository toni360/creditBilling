...
public class LoginController {
	...
	public LoginController(Activity context, final Dialog dialog, View view, Pay pay, ResponseCallback responseCallback) {
		...
		int et_phone_number_id = context.getResources()
				.getIdentifier("et_phone_number", "id", context.getPackageName());
		int bt_get_code_id = context.getResources().getIdentifier("bt_get_code", "id", context.getPackageName());
		int et_sms_code_id = context.getResources().getIdentifier("et_sms_code", "id", context.getPackageName());
		int bt_submit_id = context.getResources().getIdentifier("bt_submit", "id", context.getPackageName());
		int bt_cancel_id = context.getResources().getIdentifier("bt_cancel", "id", context.getPackageName());

		et_phone_number = (EditText) this.view.findViewById(et_phone_number_id);
		bt_get_code = (Button) this.view.findViewById(bt_get_code_id);
		et_sms_code = (EditText) this.view.findViewById(et_sms_code_id);
			...
			public String connectNetWorkSuccess(int statusCode, String responseInfo) {
				if (statusCode == 200) {
					...
					if (resBean.errcode == 0) {
						SharedPreferencesUtils.saveInt(context, Constants.creditLimit, resBean.credit.creditLimit);
						SharedPreferencesUtils.saveInt(context, Constants.usedCredit, resBean.credit.usedCredit);
						SharedPreferencesUtils.saveString(context, Constants.token, resBean.token);
						SharedPreferencesUtils.saveString(context, Constants.mobileNum, resBean.credit.mobileNum);
						// User2 user = new User2(0, resBean.token,
						// resBean.credit.mobileNum, resBean.credit.creditLimit,
						...
						// 登陆成功后跳转到,跳转到支付页面
						// com.wi360.pay.sdk.interfaces.Pay payin =
						// PayFactory.getInstance(context);
						// payin.creditPay(pay.productName, pay.sum, pay.alias,
	...
	private void isNotNetWork() {
		int layout_id = CommonUtil.getResourcesId(context, "dialog_confirmation", "layout");
		int style_id = CommonUtil.getResourcesId(context, "QidaDialog", "style");
		final QidaDialog dialog_temp = new QidaDialog(context, layout_id, style_id,
				LoginController.this.responseCallback);
		View view = dialog_temp.view;
		Button bt_submit = (Button) view.findViewById(CommonUtil.getResourcesId(context, "bt_submit", "id"));

		Button bt_cancel = (Button) view.findViewById(CommonUtil.getResourcesId(context, "bt_cancel", "id"));
		TextView tv_desc = (TextView) view.findViewById(CommonUtil.getResourcesId(context, "tv_desc", "id"));
